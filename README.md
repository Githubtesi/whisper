...
```
※2026/04の最新版cu12を選択しています。使用しているCUDAのバージョンに合わせて cu11 または cu13 を選択してください。
...
### 6. exeファイルでGPUでエラーが出たら
ビルドされた .exe で cuda（GPU） を選択してエラーが出る場合、以下のDLLファイルが app.exe と同じフォルダに足りない可能性があります。

もしエラー（cudnn_ops_infer64_8.dll が見つかりません 等）が出た場合は、Pythonの仮想環境内にある以下の場所から、DLLファイルを app.exe があるフォルダへ直接コピーしてください。
